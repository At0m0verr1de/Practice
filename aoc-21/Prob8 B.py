dataset = """dcagfb cbegda cabdf bagedf dcb cd dcbegaf aebcf bgdaf dgcf | dacebgf cbafd dbacf cdgf
...
dbcg fbagc bcf bc cdaebgf aegbdf faecg fdcaeb cdafbg afbgd | adfbg fadebgc dgbafc febagd""".split("\n")
...
for _ in dataset:
    strin = ''
    text = _.split(" | ")
    connect = text[0].split()
    code = text[1].split()

    for k in range(0, len(connect)):
        temp2 = list(connect[k])
        temp2.sort()
        connect[k] = "".join(temp2)
    ...
    len2 = [x for x in connect if len(x) == 2]
    len3 = [x for x in connect if len(x) == 3]
    len4 = [x for x in connect if len(x) == 4]
    len5 = [x for x in connect if len(x) == 5]
    len6 = [x for x in connect if len(x) == 6]
    len7 = [x for x in connect if len(x) == 7]

    positionKeys = {1:'', 2:'', 3:'', 4:'', 5:'', 6:'', 7:''}
    numbers = {0:'', 1:len2[0], 2:'', 3:'', 4:len4[0], 5:'', 6:'', 7:len3[0], 8:len7[0], 9:''}

    #  Finding string for 6
    for k in numbers[1]:
        for j in len6:
            if k not in j:
                numbers[6] = j
                break
    len6.remove(numbers[6])
    
    #  Finding 1 pos segment letter
    positionKeys[1] = numbers[7].replace(numbers[1], '')

    #  Finding 2, 3 pos segment letter
    for j in numbers[6]:
        if j in numbers[1]:
            positionKeys[3] = j
    positionKeys[2] = numbers[1].replace(positionKeys[3], '')

    #  Finding 7, 5 pos segment letter
    temp = []
    for j in len6[0]:
        if j not in len6[1]:
            temp.append(j)
    ...
    for j in temp:
        if j in numbers[4]:
            positionKeys[7] = j
            temp.remove(j)
    positionKeys[5] = temp[0]

    #  Finding 6 pos segment letter
    for j in numbers[4]:
        if j not in positionKeys.values():
            positionKeys[6] = j
            break

    #  Finding 4 pos segment letter
    for j in 'abcdefg':
        if j not in positionKeys.values():
            positionKeys[4] = j
            break

    #  Finally finding all number representations
    numbers[0] = positionKeys[1] + positionKeys[2] + positionKeys[3] + positionKeys[4] + positionKeys[5] + positionKeys[6]
    numbers[2] = positionKeys[1] + positionKeys[2] + positionKeys[7] + positionKeys[5] + positionKeys[4]
    numbers[3] = positionKeys[1] + positionKeys[2] + positionKeys[7] + positionKeys[3] + positionKeys[4]
    numbers[5] = positionKeys[1] + positionKeys[6] + positionKeys[7] + positionKeys[3] + positionKeys[4]
    numbers[9] = positionKeys[1] + positionKeys[2] + positionKeys[3] + positionKeys[4] + positionKeys[6] + positionKeys[7]
    ...
    key_list = list(map(str, list(numbers.keys())))
    val_list = list(numbers.values())

    for word in code:
        strin += key_list[val_list.index(word)]
    print(strin)